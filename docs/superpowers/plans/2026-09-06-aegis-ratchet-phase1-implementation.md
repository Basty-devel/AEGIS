...
**Before writing `RatchetState::to_bytes`/`from_bytes`, resolve an open blocker.** `Brainpool512SecretKey` and `MlKem1024KeyPair` (from `aegis-crypto`) expose no way to export their **private** key bytes — by design, since that would work against their own zeroization guarantees. Serializing `RatchetState` therefore cannot round-trip `self_ratchet_ecdh`/`self_ratchet_kem` through those types' own APIs as they stand today.

This is a real gap the design doc's §1 ("`RatchetState` serializes to/from a flat byte format") didn't anticipate at this precision, surfaced here because Task 6 is the first task that actually has to serialize the struct. Two ways to close it — **stop and get your human partner's decision between them before writing any `to_bytes`/`from_bytes` code**, since it changes what the rest of this task's code looks like:

1. **Add raw private-scalar/seed export to `aegis-crypto`** (a small addition: `Brainpool512SecretKey::to_bytes(&self) -> Zeroizing<[u8; 64]>` returning the raw scalar via `elliptic_curve::SecretKey::to_bytes()`, which already exists on the wrapped type per `ecdh.rs`; `MlKem1024KeyPair` would need its generating seed retained at construction time to support the same, since `ml_kem::DecapsulationKey` has no public raw-export either). This is the cleanest fix, but it's a change to an already-published crate (`aegis-crypto` 0.1.3) — out of this plan's scope to decide unilaterally. **Flag this to your human partner before implementing Task 6**: does adding these two export methods to `aegis-crypto` (a small, additive, non-breaking change — new methods, nothing removed) get a green light, or should Task 6 use option 2 instead?
2. **Store raw seeds inside `RatchetState` directly, generated by this crate rather than via `aegis-crypto`'s constructors.** Instead of holding `Brainpool512SecretKey`/`MlKem1024KeyPair`, `RatchetState` holds the raw seed bytes it generated them from (`Zeroizing<[u8; 32]>` for a fresh scalar sample seed, `Zeroizing<[u8; 64]>` for the ML-KEM seed — matching `MlKem1024KeyPair::generate`'s own internal seed size), and reconstructs the actual keys on demand inside each function that needs them (`aegis_crypto` doesn't currently expose a "from this raw seed" constructor for `Brainpool512SecretKey` either — only `MlKem1024KeyPair` does, via `MlKem1024::from_seed`, which `aegis-crypto`'s own `kem.rs` already calls). This avoids touching `aegis-crypto` at all but means duplicating some of its key-generation logic here.

**Do not proceed past this point in Task 6 without that decision** — it changes what the rest of this task's code looks like. Once decided, write `to_bytes`/`from_bytes` covering every field (`root_key`, `sending_chain`/`receiving_chain` as a presence byte plus 64 bytes each, the self-ratchet key material per whichever option was chosen, `peer_ratchet_ecdh_public`, `peer_ratchet_kem_public`, and the three `u32` counters big-endian), each guarded the same truncation-safe way `PreKeyBundle::from_bytes` (Task 3) is, and re-run this task's `state::` tests.
...
- **Task 6 has a real open decision** (raw private-key export from `aegis-crypto` vs. seed-based reconstruction in `aegis-ratchet`) that could not be resolved without your human partner's input — it's flagged inline in that task rather than silently decided, per this plan's own no-placeholders rule. Surface it before writing that task's `to_bytes`/`from_bytes`.
